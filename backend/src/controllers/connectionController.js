...
export const removeConnection = async (req, res) => {
  try {
    ...
    if (connection.requester.toString() !== currentUser._id.toString() && 
        connection.recipient.toString() !== currentUser._id.toString()) {
      return res.status(403).json({ message: 'Not authorized to remove this connection' });
    }

    // Determine the other user
    const otherUserId = connection.requester.toString() === currentUser._id.toString() ? connection.recipient : connection.requester;

    // Delete the connection document
    await Connection.findByIdAndDelete(connectionId);

    // Remove each other from their UserProfile.connections arrays
    await UserProfile.findByIdAndUpdate(currentUser._id, {
      $pull: { connections: otherUserId }
    });
    await UserProfile.findByIdAndUpdate(otherUserId, {
